...
| 室外最终 ID-only 与框点修正 | 团队既有 H100 双向关联、片段拼接与定稿脚本；4090 框点修正脚本 | 迁入 `legacy/outdoor/final_id_only`；参数沿用已选流程，路径由配置提供，不替代 SAM 原版目录 |
...
2. SAM 对照路线原始算法保留；统一导入时按同帧同 ID 合并重放结果。室外最终路线另纳入双向关联、片段拼接与仅回写 ID 的定稿流程。
...
首次集成时的逐文件来源哈希见 [evidence/source_manifest.json](evidence/source_manifest.json)；后续修改由 Git 提交记录追溯。
# TrackEval 评测依赖
