...
我们提出 Hive-Q²K Dual，其核心思想是：同一实体的证据可以跨时间积累，而这些证据对不同任务的贡献，应由相应的可观测性与可靠程度共同决定。方法首先通过双域感知、密度残差、身体结构与时序关联形成时空实体，再以群组与实例两个粒度的质量估计及校准专家证据调节监督强度。在此基础上，可用实体被组织为空间密度、巢口通量、持续近邻网络与群体时序，并与环境条件共同形成具体的蜂学复核问题。跨层矛盾和解释价值进一步引导主动复核、分布校准、轨迹增强与渐进学习，使一次修正同时作用于当前标注、下一轮学习经验及知识支持度。由此，方法将视觉观测与蜂学解释连接为相互校正的数据飞轮。
...
这一洞察也赋予跨层矛盾新的价值。当局部计数升高、身份突然增多而真实活动变化有限时，相互不协调的证据可以帮助定位值得复核的区段；当实体观测一致且群体结构与环境共同改变时，同一区段又可以进入蜂学解释。复核的对象因此从单个低分预测扩展到具有下游影响的实体关系与行为时段。本文据此研究：**如何以时空实体维持证据联系，以质量估计决定监督强度，再以群体解释确定下一轮的数据获取方向。**
...
完成观测校正后，我们以外观相似性、运动连续性和弱姿态约束建立身份联系。ByteTrack 表明，低分响应仍可能通过已有轨迹获得有效支持；[18](https://link.springer.com/chapter/10.1007/978-3-031-20047-2_1) 这为结合历史上下文判断当前观测提供了直接依据。短时缺失的个体保留身份记忆，边缘区域则根据个体进出的空间先验采用较短记忆。对于前后已经关联到同一身份的观测，以两端位置约束缺口内的几何恢复：
...
经过实体重建与质量估计，视觉结果已经具有位置、姿态、身份及可信程度，因而能够支持从个体到群体的行为抽象。我们以 BEEG 组织这一过程，将证据划分为直接视觉观测、轨迹派生量和组合事件三个层次。位置与身体轴构成第一层；速度、转向与区域停留构成第二层；持续交互、通量变化和环境相关的群体事件则由前两层组合形成。蜜蜂定向放香的研究表明，空间间距、身体方向与个体关系能够承载群体组织过程，[13](https://pmc.ncbi.nlm.nih.gov/articles/PMC8020754/) 因而本文以关系性表征连接个体运动与蜂群解释。
...
通量以单位观测时间内的穿越次数表示，与空间密度共同构成群体活动的空间与边界描述。交通变化由此获得可比较的时间尺度，并可与同期环境和采集记录联系起来，确定值得进一步观察的区段。
...
群体变化的含义依赖其发生条件。热环境研究表明，蜜蜂的飞行运动学与能量分配会随环境温度和负载共同改变；[14](https://pmc.ncbi.nlm.nih.gov/articles/PMC10823226/) 因此，本文将活动比例、速度及空间密度的变化与同期温度、风雨记录对齐，形成针对具体时间窗的环境响应问题。相关区段再联系到水源、遮阳和通风条件的现场检查，使视觉测量能够服务于明确的管理判断。

对于疑似暴露相关变化，农药风险研究指出，觅食特征与景观条件共同影响蜜蜂接触农药的机会和程度。[15](https://www.nature.com/articles/s41559-023-01990-5) 本文据此将活动和巢口交通的异常区段与施药时间、作物背景、死蜂记录及同期天气组织到同一时间线上。视觉结果负责描述变化的位置、持续时间与幅度，暴露情境和专业判读进一步决定该变化的蜂学含义。

对分蜂相关时段，群体活动、空间重组与巢口交通共同用于选择需要调阅长时观测的区段，并由王台核验、蜂箱重量及可获得的振动记录补充解释条件。这样，一个局部活动变化能够转化为具有时间依据的复核问题，而专家判断又能够指向更值得检查的实体关系和上游标注。

上述场景统一遵循“视觉测量—条件关联—专家复核”的推断顺序。文献提供具有生物学依据的联系，环境记录限定当前问题的发生条件，人工判读赋予事件语义。由这一过程形成的知识进一步参与数据选择，使蜂学解释既连接生产问题，也决定下一轮观测应当集中于哪些时段。
...
**由群体解释反向引导的知识反馈。** 我们以可用实体证据构建空间密度、巢口通量、持续近邻网络与群体时序，再将环境条件和蜂学问题纳入复核价值。跨层冲突指出可能的测量误差，条件化解释指出具有生物学价值的观察时段；人工复核同时修正标签和知识支持度，并通过分布校准与渐进课程作用于下一轮学习。群体解释由此参与监督获取，视觉能力与蜂学知识形成相互校正的关系。
...
**知识反馈的验证**关注群体解释能否提高后续监督获取的价值。对照实例不确定性选择、加入群体变化线索、再加入环境条件与蜂学复核的选择方式，在相同复核名额下统计有效修正、事件确认及下一轮性能变化。同时，以人工过线事件、代表区段的个体观测和交互判读评价通量、密度与时间网络；以具有时间依据的蜂学事件检验解释准确性、误报及其对管理决策的支持。CEBRA 通过正确条件与打乱标签的比较检验表示所对应的行为结构，[16](https://www.nature.com/articles/s41586-023-06031-6) 本文据此在验证中加入时间错位或关系置乱的对照，考察复核收益是否依赖真实的实体关系和环境对应。这样的设计将指标的测量有效性、解释的条件依赖性，以及解释对学习的贡献分别对应到可检验的问题。
...
[14] Glass et al. (2024). [Flying, nectar-loaded honey bees conserve water and improve heat tolerance by reducing wingbeat frequency and metabolic heat production](https://pmc.ncbi.nlm.nih.gov/articles/PMC10823226/). *Proceedings of the National Academy of Sciences*, 121, e2311025121. doi:10.1073/pnas.2311025121.

[15] Knapp et al. (2023). [Ecological traits interact with landscape context to determine bees’ pesticide risk](https://www.nature.com/articles/s41559-023-01990-5). *Nature Ecology & Evolution*, 7, 547–556. doi:10.1038/s41559-023-01990-5.

[16] Schneider, Lee, Mathis (2023). [Learnable latent embeddings for joint behavioural and neural analysis](https://www.nature.com/articles/s41586-023-06031-6). *Nature*, 617, 360–368. doi:10.1038/s41586-023-06031-6.

[17] Cao et al. (2023). [Observation-Centric SORT: Rethinking SORT for Robust Multi-Object Tracking](https://openaccess.thecvf.com/content/CVPR2023/html/Cao_Observation-Centric_SORT_Rethinking_SORT_for_Robust_Multi-Object_Tracking_CVPR_2023_paper.html). *CVPR*, 9686–9696.

[18] Zhang et al. (2022). [ByteTrack: Multi-object Tracking by Associating Every Detection Box](https://link.springer.com/chapter/10.1007/978-3-031-20047-2_1). *ECCV*, 1–21. doi:10.1007/978-3-031-20047-2_1.
