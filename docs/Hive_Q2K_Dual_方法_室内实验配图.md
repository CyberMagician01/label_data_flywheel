...
这里的联系具有明确的方向：时空实体为质量判断提供上下文，可观测性决定相应实体能够支持哪些行为测量，行为知识为复核指出具有解释价值的时段。反馈因而作用于三个对象：标签修正改变“学什么”，分布与增强改变“重点看到什么”，知识更新改变“下一轮重点检查什么”。以下先说明实体证据如何形成及其能够支持的判断，再展开复核结果如何返回学习。
...
域内标注器负责形成可供复核和学习的视觉观测。室内由 YOLO 获取检测候选，关键点模型提供头尾姿态，外观、位移与身体轴共同支持个体关联；室外采用 SAM 2 框架的 SAM2.1 模型进行时序传播，并以新增检测提示与几何吸附校正传播位置。[11](https://proceedings.iclr.cc/paper_files/paper/2025/hash/45c1f6a8cbf2da59ebf2c802b4f742cd-Abstract-Conference.html) 头尾姿态通过同帧一对一匹配附着于对应实体，使不同来源的空间范围、身份和身体方向具有明确的归属。密度与时序证据随后围绕这些实体补充和检验观测。
...
密度补充的局部表现见图 2。同一帧中的两个区域分别新增一个检测候选，说明局部复检能够改变基础检测对身体区域的覆盖。新增候选与邻近观测之间的关系仍需进一步判断，这也引出下一节的身体结构校正。

![室内密度复检的局部观测示例](figures/density_reobservation.svg)

*图 2. 室内密度复检的两个局部示例，取自标注员 03 对应的 B-5-3 第 665 帧。左列为人工标注，右列为密度复检后的模型预测；绿色表示人工框，紫色表示基础检测，青色表示新增候选。两个区域的预测框数量分别由 11 增至 12、由 6 增至 7，预测显示阈值为 0.25。图像来自已有室内密度复检实验，展示候选的空间位置及其与原有预测的关系。*
...
图 3 进一步展示可见性约束对插值结果的作用。在相同直接观测和插值输入下，20% 的 IoMin 阈值保留 98 个插值候选，暂时隐藏 108 个冲突候选，398 个直接检测框全部保留。全局图与同位置放大图共同显示，这一规则在保留部分空间补充的同时，减少了插值造成的局部交叠。身份的持续联系仍由前后观测约束，当前帧显示多少框则由可分辨的空间证据决定。

![室内插值可见性控制的同帧全局对照](figures/interpolation_visibility.svg)

*图 3. B-5-3 第 6400 帧（213.33 秒）的插值重叠阈值对照。左右使用相同的 398 个直接检测和 206 个插值候选，仅改变插值可见性约束；右列采用 IoMin 20% 与最小交集面积 16 像素平方。青色为直接检测，黄色为插值框，白色虚线标出下方放大区域。源实验为固定输入的重叠阈值比较；该帧距最近人工标注帧 1865 帧，图中数量表示算法的保留结果。*
...
其中，$u_i$ 为预测不确定性，可由实体质量的补量 $1-q_i$ 表达；$\delta_i$ 为第 4.2 节保留的专家分歧，$r_i$ 为稀缺程度，$g_i$ 为知识增益评分，$h_i$ 为跨层修正价值，$b_i$ 为行为解释价值，$c_i$ 为复核成本。选择过程以此评分为基础，并结合特征空间中的覆盖距离逐步选取互补样本，使固定复核名额同时覆盖高价值问题与不同场景。
...
经过复核与归因，下一轮已经具有修正后的监督和需要重点覆盖的问题切片。目标分布校准进一步决定这些样本在学习中获得多少关注，使人工修正能够影响后续经验的组成。我们以观测域、视频、目标尺度、密度、遮挡、方向及质量构成数据切片，并将应用覆盖、评价覆盖和当前误差共同纳入目标分布。
...
复核更新了监督内容，分布校准决定了样本出现的频率，增强进一步改变了这些样本的观察条件。联合学习将三者汇入同一目标，并使不同层级的监督随其成熟程度逐步发挥作用。我们将每轮训练组织为质量加权的双域多任务学习。设由校准权重诱导的域内采样分布为 $P_{r,d}^{w}$，任务层级集合为 $\mathcal L$，课程阶段为 $s$，则其目标可表述为
...
这里，$P_{r,d}^{w}$ 对应第 6.4 节校准后的学习机会，$m_{i\ell}$ 由实体的任务可观测性决定，$q_{i\ell}$ 承接第 4 节的可靠性判断，$\lambda_{\ell,s}$ 则控制不同阶段中的任务强度。四者分别对应分布、观测、质量与课程，使数据选择和监督利用在同一个学习目标中衔接。经校准的软正例以相应强度参与训练，经确认的行为样本则为行为状态学习提供语义监督。
