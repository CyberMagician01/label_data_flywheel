...
ROOT = Path("/hy-tmp/bee26/beeposetrack_y_20260827")
DATASET = ROOT / "datasets" / "y_unified_20260901_labelme5_a1_strict_v8_annotator01_val_03_test_poseonly_posegeo_dedup"
RUN_DIR = ROOT / "runs_unified_20260901/a1_aligned/a1_unified_strict-v8-annotator01-val-03-test-poseonly_posegeo_dedup_best_seed2026_img1280_eb16"
BASE_EVAL = ROOT / "eval_unified_20260901/a1_strict_v8_poseonly_posegeo_dedup_annotator01_val03_seed2026"
CALIB_DIR = ROOT / "eval_unified_20260901/a1_strict_v8_poseonly_posegeo_dedup_annotator01_val03_seed2026_calibration"
FROZEN_TEST_DIR = ROOT / "eval_unified_20260901/a1_strict_v8_poseonly_posegeo_dedup_annotator01_val03_seed2026_calibrated_test"
...
def main() -> None:
    ...
    parser.add_argument("--base-eval")
    parser.add_argument("--calib-dir")
    parser.add_argument("--frozen-test-dir")
    args = parser.parse_args()

    ROOT = Path(args.root)
    dataset = Path(args.dataset) if args.dataset else DATASET
    run_dir = Path(args.run_dir) if args.run_dir else RUN_DIR
    base_eval = Path(args.base_eval) if args.base_eval else BASE_EVAL
    calib_dir = Path(args.calib_dir) if args.calib_dir else CALIB_DIR
    frozen_test_dir = Path(args.frozen_test_dir) if args.frozen_test_dir else FROZEN_TEST_DIR

    py = args.python
    best = run_dir / "weights/best.pt"
    manifest = dataset / "dataset_manifest.jsonl"
    for required in (best, manifest, dataset / "val_images.txt", dataset / "test_images.txt"):
    ...
    if not val_raw.exists():
        run([
            ...
            "768",
            "--model-name",
            "BeePoseTrack-Y-A1-Unified-Aligned-PoseOnly-Calib",
        ])
    ...
    if not test_raw.exists():
        ...
        run([
            ...
            "768",
            "--model-name",
            "BeePoseTrack-Y-A1-Unified-Aligned-PoseOnly-Calibrated",
        ])
