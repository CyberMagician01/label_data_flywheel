...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--run-name",
        default=None,
        help="Optional explicit training run directory name. Defaults to the reproducible data/init/seed tag.",
    )
    parser.add_argument(
        "--supervision-mode",
...
def write_alignment_report(path: Path, args: argparse.Namespace, stats: dict[str, Any], data_yaml: Path, run_dir: Path, source_lists: dict[str, Path], manifest: Path) -> None:
...
def main() -> None:
    ...
    eval_name = args.eval_name or f"a1_strict_v8_annotator01_val03_seed{args.seed}"
    ...
    name = args.run_name or f"a1_unified_{data_tag}_{tag}_seed{args.seed}_img{args.imgsz}_eb{args.nbs}"
    ...
    if not training_complete(run_dir, args.epochs):
        ...
        run(
            [
                args.python,
                str(args.y_root / "tools" / "train_yolo_pose_aligned.py"),
                "--data",
                str(data_yaml),
                "--model",
                str(train_model),
