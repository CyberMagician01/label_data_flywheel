...
class YAMLConfig(BaseConfig):
    ...
    def get_optim_params(cfg: dict, model: nn.Module):
        ...
        for pg in cfg['params']:
            ...
            overlap = visited.intersection(params)
            if overlap:
                names = ', '.join(sorted(overlap))
                raise ValueError(f'optimizer parameter groups overlap: {names}')
            pg['params'] = params.values()
            param_groups.append(pg)
            visited.update(params)

        names = {k for k, v in model.named_parameters() if v.requires_grad}

        if len(visited) < len(names):
            unseen = names - visited
            params = {k: v for k, v in model.named_parameters() if v.requires_grad and k in unseen}
            param_groups.append({'params': params.values()})
            visited.update(params)

        if visited != names:
            missing = ', '.join(sorted(names - visited))
            extra = ', '.join(sorted(visited - names))
            raise ValueError(f'invalid optimizer parameter coverage; missing=[{missing}], extra=[{extra}]')
    ...
    def reset_cfg(self):
        ...
        for ops in [train_ops, val_ops]:
            for op in ops:
                ...
                elif t in ("Resize", "LetterBox", "PrepareTemporalFrames"):
