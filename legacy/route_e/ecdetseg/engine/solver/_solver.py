...
class BaseSolver(object):
    ...
    def load_tuning_state(self, path: str):
        ...
        initialization = getattr(self.cfg, 'bee_e_initialization', None) or {}
        reinitialized_groups = set(initialization.get('reinitialize_output_heads', []))
        if reinitialized_groups:
            expected_groups = {'category', 'quality', 'density', 'endpoints'}
            if reinitialized_groups != expected_groups:
                raise ValueError(
                    'BeePoseTrack-E output-head reinitialization must cover exact '
                    'category/quality/density/endpoints groups.'
                )
            pretrain_state_dict = dict(pretrain_state_dict)
            removed = [
                name for name in pretrain_state_dict
                if self._bee_e_output_head_group(name) in reinitialized_groups
            ]
            for name in removed:
                del pretrain_state_dict[name]
            print(
                'Reinitialized BeePoseTrack-E output heads instead of loading '
                f'{len(removed)} pretrained tensors: {sorted(reinitialized_groups)}'
            )

        pretrain_state_dict = self._expand_scene_detection_heads(
    ...
    @staticmethod
    def _bee_e_output_head_group(name):
        """Map a state key to the output head that must start task-fresh."""
        name = str(name).removeprefix('module.')
        if name.startswith((
            ...
            return 'category'
        if name.startswith('decoder.dec_quality_head.'):
            return 'quality'
        if name.startswith('density_head.'):
            return 'density'
        if name.startswith((
            'decoder.endpoint_decoder.',
            'decoder.dec_keypoint_head.',
            'decoder.dec_visibility_head.',
            'decoder.dec_keypoint_refine_head.',
            'decoder.dec_local_visibility_head.',
        )):
            return 'endpoints'
        return None

    @staticmethod
    def _expand_scene_detection_heads(cur_state_dict, pretrain_state_dict):
        """Copy a shared pretrained detection head into both scene branches."""
        expanded = dict(pretrain_state_dict)
        copied = []
        for current_name, current_value in cur_state_dict.items():
            if current_name in expanded:
                continue
            source_name = None
            for scene_index in (0, 1):
                marker = f'.heads.{scene_index}.'
                if marker in current_name:
                    source_name = current_name.replace(marker, '.', 1)
                    break
            if source_name is None or source_name not in pretrain_state_dict:
                continue
            source_value = pretrain_state_dict[source_name]
            if current_value.shape != source_value.shape:
                continue
            expanded[current_name] = source_value
            copied.append(current_name)
        if copied:
            print(
                'Expanded shared pretrained detection tensors into indoor/outdoor '
                f'branches: {len(copied)} tensors.'
            )
        return expanded
