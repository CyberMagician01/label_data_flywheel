...
class ConvPyramidPatchEmbed(nn.Module):
    def __init__(self, embed_dim=192, patch_size=16, act='relu'):
        ...
        self.p2_channels = channels[1]
        self.p2_stride = 4
        self.p3_channels = channels[2]
        self.p3_stride = 8
        
        self.proj = nn.Conv2d(channels[-1], embed_dim, kernel_size=3, stride=2, padding=1)

    def forward_with_stem(self, x):
        p2 = None
        p3 = None
        for stage_idx, conv in enumerate(self.convs):
            x = conv(x)
            if stage_idx == 1:
                p2 = x
            if stage_idx == 2:
    ...
    def forward_shallow(self, x):
        """Return only convolutional P2/P3; never enter the ViT blocks."""
        p2 = None
        for stage_idx, conv in enumerate(self.convs):
            x = conv(x)
            if stage_idx == 1:
            ...
            if stage_idx == 2:
                return p2, x
        raise RuntimeError('ConvPyramidPatchEmbed must expose stride-4 and stride-8 stages.')

    def forward(self, x):
        x, _, _ = self.forward_with_stem(x)
        return x
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x, return_stem_p2=False):
        outs = []
        if return_stem_p2:
            if not hasattr(self.patch_embed, 'forward_with_stem'):
                raise RuntimeError('Native P2 requires ConvPyramidPatchEmbed.')
            x_embed, stem_p2, stem_p3 = self.patch_embed.forward_with_stem(x)
        else:
        ...
        for i, blk in enumerate(self.blocks):
            ...
            if i in self.return_layers:
                outs.append(x[:, 1:])
        return (outs, stem_p2, stem_p3) if return_stem_p2 else outs
...
class ViTAdapter(nn.Module):
    ...
    def __init__(
        ...
        num_levels=3,
        use_native_p2=False,
        enable_dual_domain_norm=False,
        freeze_domain_bn_stats=False,
        domain_adapter_rank=0,
        enable_domain_layernorm=False,
        log_feature_contract=False,
        ...
        **kwargs
        ...
        self.use_native_p2 = use_native_p2
        self.enable_dual_domain_norm = bool(enable_dual_domain_norm)
        self.freeze_domain_bn_stats = bool(freeze_domain_bn_stats)
        self.domain_adapter_rank = int(domain_adapter_rank)
        self.enable_domain_layernorm = bool(enable_domain_layernorm)
        self.log_feature_contract = log_feature_contract
        self._feature_contract_logged = False
        
        if num_levels not in (3, 4):
            raise NotImplementedError("ViTAdapter supports three levels or an extra stride-4 P2 level.")

        if isinstance(proj_dim, (list, tuple)):
            if len(proj_dim) != num_levels:
                raise ValueError('proj_dim list length must equal num_levels.')
            self.proj_dim = list(proj_dim)
        else:
            self.proj_dim = [proj_dim or embed_dim] * num_levels

        self.projector = nn.ModuleList([ConvNormLayer_fuse(embed_dim, dim, kernel_size=1, stride=1) for dim in self.proj_dim])
        if self.use_native_p2:
            if self.num_levels != 4:
                raise ValueError('Native P2 is defined for a four-level P2-P5 pyramid.')
            if not hasattr(self.backbone.patch_embed, 'p2_channels'):
                raise ValueError('Native P2 requires a patch embed that exposes p2_channels.')
            p2_in_channels = self.backbone.patch_embed.p2_channels
            p2_out_channels = self.proj_dim[0]
            self.native_p2_adapter = nn.Sequential(
                nn.Conv2d(p2_in_channels, p2_out_channels, 1, bias=False),
                nn.BatchNorm2d(p2_out_channels),
                nn.SiLU(),
                nn.Conv2d(
                    p2_out_channels,
                    p2_out_channels,
                    3,
                    padding=1,
                    groups=p2_out_channels,
                    bias=False,
                ),
                nn.BatchNorm2d(p2_out_channels),
                nn.SiLU(),
            )
            self.history_p3_adapter = nn.Sequential(
                nn.Conv2d(
                    self.backbone.patch_embed.p3_channels,
                    self.proj_dim[1],
                    1,
                    bias=False,
                ),
                nn.BatchNorm2d(self.proj_dim[1]),
                nn.SiLU(),
                nn.Conv2d(
                    self.proj_dim[1], self.proj_dim[1], 3, padding=1,
                    groups=self.proj_dim[1], bias=False,
                ),
                nn.BatchNorm2d(self.proj_dim[1]),
                nn.SiLU(),
            )

            if self.enable_dual_domain_norm:
                # The route requires explicit RGB/IR statistics in the convolutional
                # stem and the two highest-resolution P2/P3 feature paths.
                for stem_stage in self.backbone.patch_embed.convs[:3]:
                    stem_stage.norm = DomainSpecificBatchNorm2d.from_batch_norm(
                        stem_stage.norm, freeze_running_stats=self.freeze_domain_bn_stats,
                    )
                for adapter in (self.native_p2_adapter, self.history_p3_adapter):
                    for index, layer in enumerate(adapter):
                        if isinstance(layer, nn.BatchNorm2d):
                            adapter[index] = DomainSpecificBatchNorm2d.from_batch_norm(
                                layer, freeze_running_stats=self.freeze_domain_bn_stats,
                            )
                # projector[1] produces the semantic P3 level in a native-P2 pyramid.
                self.projector[1].norm = DomainSpecificBatchNorm2d.from_batch_norm(
                    self.projector[1].norm,
                    freeze_running_stats=self.freeze_domain_bn_stats,
                )

            if self.domain_adapter_rank > 0:
                self.domain_p2_adapter = DomainLowRankAdapter2d(
                    self.proj_dim[0], self.domain_adapter_rank,
                )
                self.domain_p3_adapter = DomainLowRankAdapter2d(
                    self.proj_dim[1], self.domain_adapter_rank,
                )

        if self.enable_domain_layernorm:
            replace_layer_norms(self.backbone)
        self.feature_sources = (
            ('conv_stem_stride4', 'vit_stride8', 'vit_stride16', 'vit_stride32')
            if self.use_native_p2 else
            tuple(f'vit_interpolate_level_{level_idx}' for level_idx in range(self.num_levels))
        )
    ...
    def forward(self, x, return_shallow=False, domain_id=None):
        if self.enable_dual_domain_norm or self.enable_domain_layernorm or self.domain_adapter_rank > 0:
            set_explicit_domain(self, domain_id)
        
        H_c, W_c = x.shape[2] // self.patch_size, x.shape[3] // self.patch_size
        bs = x.shape[0]

        backbone_output = self.backbone(x, return_stem_p2=self.use_native_p2)
        if self.use_native_p2:
            return_layers, stem_p2, stem_p3 = backbone_output
        else:
            return_layers = backbone_output
            stem_p2 = None
            stem_p3 = None
        ...
        vit_level_count = self.num_levels - 1 if self.use_native_p2 else self.num_levels
        for i in range(vit_level_count):
            scale = 2 ** (vit_level_count - 2 - i)
        ...
        if self.use_native_p2:
            proj_feats = [self.native_p2_adapter(stem_p2)] + [
                layer(feat) for layer, feat in zip(self.projector[1:], proj_feats)
            ]
        else:
            proj_feats = [layer(feat) for layer, feat in zip(self.projector, proj_feats)]

        if self.use_native_p2 and self.domain_adapter_rank > 0:
            proj_feats[0] = self.domain_p2_adapter(proj_feats[0])
            proj_feats[1] = self.domain_p3_adapter(proj_feats[1])

        if self.use_native_p2:
            expected_p2_size = (
                math.ceil(x.shape[-2] / 4),
                math.ceil(x.shape[-1] / 4),
            )
            if proj_feats[0].shape[-2:] != expected_p2_size:
                raise RuntimeError(
                    f'Native P2 must be stride-4: expected {expected_p2_size}, '
                    f'got {tuple(proj_feats[0].shape[-2:])}.'
                )
        if self.log_feature_contract and not self._feature_contract_logged:
            metadata = []
            for source, feature in zip(self.feature_sources, proj_feats):
                stride_h = x.shape[-2] / feature.shape[-2]
                stride_w = x.shape[-1] / feature.shape[-1]
                metadata.append(
                    f'{source}: shape={tuple(feature.shape)}, '
                    f'stride=({stride_h:g},{stride_w:g}), channels={feature.shape[1]}'
                )
            print('[ViTAdapter feature contract] ' + ' | '.join(metadata))
            self._feature_contract_logged = True
            
        if return_shallow:
            if not self.use_native_p2:
                raise RuntimeError('Shallow temporal features require the native P2 backbone contract.')
            return proj_feats, [
                proj_feats[0],
                self.history_p3_adapter(stem_p3),
            ]
        return proj_feats

    def forward_shallow_history(self, x, domain_id=None):
        """History-frame contract: convolutional stem plus native P2/P3 only."""
        if not self.use_native_p2:
            raise RuntimeError('Shallow history requires the native P2 backbone contract.')
        if self.enable_dual_domain_norm or self.enable_domain_layernorm or self.domain_adapter_rank > 0:
            set_explicit_domain(self, domain_id)
        p2, p3 = self.backbone.patch_embed.forward_shallow(x)
        p2 = self.native_p2_adapter(p2)
        p3 = self.history_p3_adapter(p3)
        if self.domain_adapter_rank > 0:
            p2 = self.domain_p2_adapter(p2)
            p3 = self.domain_p3_adapter(p3)
        return [p2, p3]
