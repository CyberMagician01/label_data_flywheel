...
class SegmentationHead(nn.Module):
    def __init__(self, in_dim, num_blocks: int, bottleneck_ratio: int=1, downsample_ratio: int=4, image_size: tuple[int, int]=[640,640]):
        super().__init__()
    ...
    def forward(self, spatial_features: torch.Tensor, query_features: list[torch.Tensor], skip_blocks: bool=False) -> list[torch.Tensor]:
    ...
    def forward_export(self, spatial_features: torch.Tensor, query_features: list[torch.Tensor],  skip_blocks: bool=False) -> list[torch.Tensor]:
