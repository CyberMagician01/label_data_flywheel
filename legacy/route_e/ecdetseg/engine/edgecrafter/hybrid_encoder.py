...
class P2LocalWindowInteraction(nn.Module):
    """Non-overlapping local self-attention on the stride-4 feature map."""

    def __init__(self, channels, window_size=8, num_heads=8, mlp_ratio=2.0):
        super().__init__()
        if channels % num_heads:
            raise ValueError('P2 channels must be divisible by local-window heads.')
        ...
        relative_size = 2 * self.window_size - 1
        self.relative_position_bias = nn.Parameter(
            torch.zeros(relative_size * relative_size, self.num_heads)
        )
    ...
    def forward(self, feature):
        ...
        tokens = padded.view(
            batch, channels,
            padded_height // window, window,
            padded_width // window, window,
        ).permute(0, 2, 4, 3, 5, 1).reshape(-1, window * window, channels)

        if isinstance(self.norm1, DomainConditionedLayerNorm):
            sample_domain_id = self.norm1._checked_domain_id(batch, feature.device)
            window_count = (padded_height // window) * (padded_width // window)
            window_domain = sample_domain_id.repeat_interleave(window_count)
        else:
            window_domain = None

        residual = tokens
        if window_domain is None:
            normalized = self.norm1(tokens)
        else:
            normalized = self.norm1(tokens, domain_id=window_domain)
        ...
        attention = attention + relative_bias.permute(2, 0, 1)[None]
        attention = attention.softmax(dim=-1)
        attended = (attention @ value).transpose(1, 2).reshape_as(normalized)
        tokens = residual + self.proj(attended)
        if window_domain is None:
            normalized = self.norm2(tokens)
        else:
            normalized = self.norm2(tokens, domain_id=window_domain)
        tokens = tokens + self.mlp(normalized)

        output = tokens.reshape(
            batch, padded_height // window, padded_width // window,
...
class P2P3HighResolutionFusion(nn.Module):
    """Convolutional P3-to-P2 fusion before the ordinary FPN/PAN path."""

    def __init__(self, channels, act='silu'):
        super().__init__()
        self.p3_projection = ConvNormLayer_fuse(channels, channels, 1, 1, act=act)
        self.fusion = nn.Sequential(
            ConvNormLayer_fuse(
                channels * 2, channels * 2, 3, 1,
                g=channels * 2, act=act,
            ),
            ConvNormLayer_fuse(channels * 2, channels, 1, 1, act=None),
        )
        self.activation = get_activation(act)

    def forward(self, p2, p3):
        p3 = F.interpolate(
            self.p3_projection(p3), size=p2.shape[-2:],
            mode='bilinear', align_corners=False,
        )
        return self.activation(p2 + self.fusion(torch.cat([p2, p3], dim=1)))
...
class ConvNormLayer_fuse(nn.Module):
    ...
    def convert_to_deploy(self):
        # RGB/IR domain-specific normalization cannot be folded into a single
        # convolution without destroying the explicit domain contract.
        if not isinstance(getattr(self, 'norm', None), nn.BatchNorm2d):
            return self
...
class HybridEncoder(nn.Module):
    ...
    def __init__(self,
                 ...
                 csp_type='csp',
                 fuse_op='cat',
                 enable_p2_local_window=False,
                 p2_window_size=8,
                 p2_window_heads=8,
                 p2_window_mlp_ratio=2.0,
                 enable_p2_p3_high_resolution_fusion=False,
                 enable_dual_domain_norm=False,
                 freeze_domain_bn_stats=False,
                 enable_domain_layernorm=False,
                 activation_checkpoint_high_resolution=False,
                 ):
        super().__init__()
        ...
        self.enable_p2_local_window = bool(enable_p2_local_window)
        self.enable_p2_p3_high_resolution_fusion = bool(
            enable_p2_p3_high_resolution_fusion
        )
        ...
        self.activation_checkpoint_high_resolution = bool(
            activation_checkpoint_high_resolution
        )
        if (
            self.enable_p2_local_window
            or self.enable_p2_p3_high_resolution_fusion
        ) and (len(in_channels) < 2 or list(feat_strides[:2]) != [4, 8]):
            raise ValueError('P2 local/fusion route requires stride-4 and stride-8 inputs.')
        if self.enable_p2_local_window:
            self.p2_local_window = P2LocalWindowInteraction(
                hidden_dim, p2_window_size, p2_window_heads,
                p2_window_mlp_ratio,
            )
        if self.enable_p2_p3_high_resolution_fusion:
            self.p2_p3_high_resolution_fusion = P2P3HighResolutionFusion(
                hidden_dim, act=act,
            )
            if self.enable_dual_domain_norm:
                for module in self.p2_p3_high_resolution_fusion.modules():
                    if isinstance(module, ConvNormLayer_fuse):
                        module.norm = DomainSpecificBatchNorm2d.from_batch_norm(
                            module.norm,
    ...
    def forward(self, proj_feats):
        ...
        def high_resolution_call(module, *inputs):
            if self.training and self.activation_checkpoint_high_resolution:
                return checkpoint(module, *inputs, use_reentrant=False)
            return module(*inputs)

        if self.enable_p2_local_window:
            proj_feats[0] = high_resolution_call(
                self.p2_local_window, proj_feats[0]
            )
        if self.enable_p2_p3_high_resolution_fusion:
            proj_feats[0] = high_resolution_call(
                self.p2_p3_high_resolution_fusion,
                proj_feats[0], proj_feats[1],
            )
        ...
        for idx in range(len(self.in_channels) - 1, 0, -1):
            ...
            inner_out = (
                high_resolution_call(fpn_block, fused_feat)
                if idx <= 2 else fpn_block(fused_feat)
            )
        ...
        for idx in range(len(self.in_channels) - 1):
            ...
            out = (
                high_resolution_call(self.pan_blocks[idx], fused_feat)
                if idx <= 1 else self.pan_blocks[idx](fused_feat)
            )
            outs.append(out)
